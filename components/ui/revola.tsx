...
const ResponsiveDialogOverlay = ({
  ...
  return (
    <ResponsiveDialogOverlay
      {...props}
      className={cn(
        "sm:data-[state=open]:animate-in sm:data-[state=closed]:animate-out sm:data-[state=closed]:fade-out-0 sm:data-[state=open]:fade-in-0 fixed inset-0 z-50 bg-black/80",
        className
      )}
    />
  )
...
const ResponsiveDialogContentVariants = cva("fixed z-[9999] bg-canvas-bg-subtle", {
  variants: {
    device: {
      desktop:
        "left-1/2 top-1/2 grid max-h-[calc(100%-4rem)] w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-canvas-border shadow-sm duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:max-w-lg",
      mobile: "flex ",
    },
    direction: {
      bottom: "",
  ...
  compoundVariants: [
    {
      ...
      direction: "bottom",
      className:
        "inset-x-0 bottom-0 mt-24 h-fit max-h-[75%] flex-col rounded-t-lg border border-b-0 border-canvas-border",
    },
    {
      device: "mobile",
      direction: "top",
      className:
        "inset-x-0 top-0 mb-24 h-fit max-h-[75%] flex-col rounded-b-lg border border-b-0 border-canvas-border",
    },
    {
      device: "mobile",
      direction: "left",
      ...
      <ResponsiveDialogPortal>
        ...
        >
          {!shouldUseDialog && direction === "bottom" && (
            <div
              className={cn(
                "bg-canvas-text-contrast/25 mx-auto my-4 h-1.5 w-14 rounded-full pb-1.5 data-[vaul-handle]:h-1.5 data-[vaul-handle]:w-14 data-[vaul-handle]:pb-1.5",
                dragHandleClassName
              )}
            />
          )}
          ...
          {shouldShowCloseButton && (
            <ResponsiveDialogClose
              className={cn(
                "ring-offset-canvas-bg-subtle focus-visible:ring-canvas-border hover:from-canvas-bg-subtle/90 hover:to-canvas-bg/70 hover:bg-gradient-to-br hover:text-canvas-text-contrast data-[state=open]:from-canvas-bg-subtle/90 data-[state=open]:to-canvas-bg/70 data-[state=open]:bg-gradient-to-br data-[state=open]:text-canvas-text-contrast absolute top-4 right-4 rounded-sm opacity-70 backdrop-blur-sm transition-opacity hover:opacity-100 focus:ring-offset-2 focus:outline-none focus-visible:ring-2 disabled:pointer-events-none",
                closeButtonClassName
              )}
            >
              <X className="size-4" />
...
const ResponsiveDialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className={cn("flex flex-col gap-1.5 text-center sm:text-left text-canvas-text-contrast", className)} {...props} />
  )
}
